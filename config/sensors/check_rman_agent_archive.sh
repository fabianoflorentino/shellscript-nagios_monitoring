...
where session_recid = (select max(session_recid) from V\\$RMAN_BACKUP_JOB_DETAILS where input_type='DB FULL' or input_type='ARCHIVELOG' and status <> 'FAILED')
and input_type='DB FULL' or input_type='ARCHIVELOG'
...
and input_type='DB FULL' or input_type='ARCHIVELOG'
and start_time > sysdate-1
and session_recid = (select max(session_recid) from V\\$RMAN_BACKUP_JOB_DETAILS where input_type='DB FULL' or input_type='ARCHIVELOG' and status <> 'FAILED')
...
TRAT2_DATE_RMAN=`echo "$DATE_RMAN" | sed 's/-//g' | sed 's/START_TIME//g' | sed 's/no rows selected//g' | cut -d" " -f2`

if [ -z $TRAT2_DATE_RMAN ]
then
...
and start_time > sysdate-1
and session_recid = (select max(session_recid) from V\\$RMAN_BACKUP_JOB_DETAILS where input_type='DB FULL' and status <> 'FAILED')
order by session_recid;
EOF`
...
TRAT_SYSDATE=`echo "$SYSDATE" | sed -n 4p`

if [ -z $TRAT_DATE_RMAN ]
        then
        echo "2 Check_Rman_Archive_${BASE} - BACKUP RMAN COM FALHAS OU INEXISTENTE"
        exit 2
	
	elif [ $TRAT_DATE_RMAN == $TRAT_SYSDATE ]
	then
        echo "0 Check_Rman_Archive_${BASE} - `echo $TRAT_LOG_PRIMARY`"
        exit 0

	else
        echo "1 Check_Rman_Archive_${BASE} - `echo $TRAT_LOG_PRIMARY` "
        exit 1	
