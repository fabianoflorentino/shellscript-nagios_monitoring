...
#   echo "$retorno Oracle_Backup result=2;1;2;0;2  CRITICAL - Log nao encontrado"
    echo "$retorno Oracle_Backup_Export_${BASE} - CRITICAL - Log nao encontrado"
    exit $retorno
fi
...
if [ $flog == "S" ];then
    ...
    if [ $STATUS -eq 0 ]; then
        ...
        if [ $tipo -eq 1 ]; then
            ...
            echo "$retorno Oracle_Backup_Export_${BASE} result=2;1;2;0;2 CRITICAL - Backup falhou - $LOG "-" $RESULTADO"

        else

            echo "$retorno Oracle_Backup_Export_${BASE} result=2;1;2;0;2 CRITICAL - Compressao falhou - $LOG "-" $RESULTADO"
        fi
        ...
        if [ $tipo -eq 1 ]; then
            ...
            echo "$retorno Oracle_Backup_Export_${BASE} result=0;1;2;0;2 OK - Backup - $LOG "-" $RESULTADO"

        else

            echo "$retorno Oracle_Backup_Export_${BASE} result=0;1;2;0;2 OK - Compressao - $LOG "-" $RESULTADO"
        fi
    ...
    #warning
    retorno=1
    echo "$retorno Oracle_Backup_Export_${BASE} result=1;1;2;0;2 WARN - Log encontrado fora da faixa de backup: $LOG"
