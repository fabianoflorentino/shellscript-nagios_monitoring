...
USR_NUVEM=
PASSW_NUVEM=
HORA_BKP_NUVEM=21000
HOME_NUVEM=/home/$USR_NUVEM
LOCK=$(ps aux | grep  rsync | grep $HOST_NUVEM | grep $BASE | head -1 | awk '{print $1}')
...
Status_Msg (){
        FILE=$(ls -lhtr $DIR_BKP | grep $HORA_BKP_NUVEM | grep backup_db | grep $BASE | tail -1 | awk '{print $9}')
...
Test_Return_Transmition () {
        ...
        /bin/bash $DIR_SCRIPT/copy_backup_"$BASE"_nuvem.sh
        Lock_Copy_Backup_Nuvem
}

Test_Find_File () {

FIND_FILE_BACKUP=$(find $DIR_BKP -maxdepth 1 -type f -name "b*$BASE*$HORA_BKP_NUVEM*.$FILE_EXT" -mtime -1 -print)
...
FIND_LOG_RSYNC=$(find $DIR_LOG/rsync_$BASE.log -mtime -1)
if [ -z $FIND_LOG_RSYNC ]
        then
        MSG=$N5
        RETORNO=2
...
case $RETORNO in
        0) echo -e "0 Check_Integrity_Backup_Nuvem_$BASE result=0;1;2;0;2 $N1" && exit "$RETORNO";;
        1) echo -e "1 Check_Integrity_Backup_Nuvem_$BASE result=1;1;2;0;2 $MSG" && exit "$RETORNO";;
        2) echo -e "2 Check_Integrity_Backup_Nuvem_$BASE result=2;1;2;0;2 $MSG" && exit "$RETORNO";;
        *) echo -e "3 Check_Integrity_Backup_Nuvem_$BASE result=3;1;2;0;2 $N6" && exit "$RETORNO";;
esac
